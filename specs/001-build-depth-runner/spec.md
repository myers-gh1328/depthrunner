...
**Created**: 2026-03-10  
**Status**: Draft  
**Input**: User description: "Build Depth Runner - a client-side endless runner game set underwater, with buoyancy-based controls, air management, depth progression, zone-specific hazards, and persistent best-depth tracking."
...
As a player, I start a run and control a scuba diver that auto-swims forward while I manage buoyancy and dodge incoming hazards to survive as long as possible.
...
1. **Given** a run is active, **When** the player provides no input, **Then** the diver drifts upward due to buoyancy while the world scrolls continuously.
2. **Given** a run is active, **When** the player presses or holds the primary input, **Then** the diver kicks downward and can counter buoyancy.
...
As a player, I track depth and air in real time, collect air bubbles to extend the run, and adapt as deeper water increases difficulty and air pressure.
...
**Independent Test**: Can be fully tested by running long enough to cross depth thresholds, observing faster air depletion at depth, collecting air pickups, and validating score increases by depth reached.
...
4. **Given** a run ends, **When** final depth is calculated, **Then** depth reached in metres is shown as the run score.

---

### User Story 3 - Read Status and Replay for Improvement (Priority: P3)

As a player, I can clearly read HUD status and game states, see my personal best, and quickly restart to chase a deeper run.

**Why this priority**: Clear status feedback and replay loop increase retention and make progress meaningful across sessions.

**Independent Test**: Can be fully tested by moving through start, playing, and game-over states; verifying HUD placement/readability; setting a best depth; reloading; and confirming best depth persists.
...
2. **Given** the player is in a run, **When** viewing the HUD, **Then** current depth, air gauge, and best depth are visible without obscuring critical play space.
3. **Given** air is critically low, **When** the threshold is crossed, **Then** the air indicator changes to a critical visual warning.
4. **Given** the run ends, **When** final results are displayed, **Then** depth reached and best depth are shown with restart prompt, and a clear celebration appears if a new best is achieved.
5. **Given** a best depth was achieved previously, **When** the player returns in a later session, **Then** the best depth value is retained and displayed.
...
- Personal best storage unavailable or blocked: game remains playable and simply does not persist best depth for that session.
...
- **FR-004**: During active play, the diver MUST naturally drift upward over time when no player input is provided.
- **FR-005**: The player MUST be able to apply downward movement by holding or pressing a primary action input on desktop and touch devices.
...
- **FR-011**: Depth in metres MUST increase throughout a descending run and be displayed as the current score.
...
- **FR-014**: The game MUST include zone-specific obstacle sets:
  ...
  - Deep: anglerfish with lethal lure area, thermal vents with periodic upward bursts, and crushing-pressure bands with timed safe passage.
- **FR-015**: Contact between the diver and any lethal obstacle or hazard hit area MUST end the run immediately.
- **FR-016**: The HUD MUST show current depth (top left), best depth (top center), and air gauge (top right) during active play.
...
- **FR-019**: The game-over screen MUST show run depth, best depth, and a restart prompt.
- **FR-020**: When a run sets a new best depth, the game-over screen MUST clearly celebrate the new record.
- **FR-021**: Best depth MUST persist between sessions on the same device profile.
...
- **Diver State**: Player avatar runtime state including vertical position band, buoyancy effect, downward kick state, and alive/dead status.
- **Air State**: Current and maximum air values, depletion modifiers by depth, and critical threshold status.
- **Depth Run**: Current depth reached in metres for a run, zone classification, and final run depth on game over.
...
- **Best Depth Record**: Persisted local profile value for maximum depth achieved across sessions.
...
- Depth is the sole scoring dimension; no bonus multipliers or combo systems are included in this scope.
...
- **SC-002**: At least 90% of playtest participants can correctly identify depth, current air, and best depth from the HUD in under 3 seconds during active play.
...
- **SC-006**: Best depth persists correctly across browser restart in at least 99% of sessions where local persistence is available.
